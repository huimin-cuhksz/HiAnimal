...
@threestudio.register("jacobian-nvdiff-rasterizer")
class NVDiffRasterizer(Rasterizer):
    ...
    def forward(
        ...
        **kwargs
        ...
        batch_size = tgt_mtx.shape[0]
        mesh = self.geometry.isosurface()
        src_jacobian = self.geometry.src_jacobian
        ...
        jacobian_pred = self.geometry.face_attribute[:, 0:9]
        jacobian_pred = jacobian_pred.reshape(-1, 3, 3)
        # print(jacobian_pred)
        # print(jacobian_pred.shape,mesh.t_pos_idx.shape)
        """test the shape of jacobian"""
        # test_jacobian=src_jacobian.jacobians_from_vertices(mesh.v_pos.unsqueeze(0))
        # print(test_jacobian.shape,jacobian_pred.unsqueeze(0).shape)
        deform_vert = src_jacobian.vertices_from_jacobians(
            jacobian_pred.unsqueeze(0)
        ).squeeze(0)
        deform_vert = deform_vert + self.geometry.global_dis

        org_vert = mesh.v_pos.clone()
        deform_field = deform_vert - org_vert
        # print(torch.norm(deform_field))
        ...
        deform_vpos_clip = self.ctx.vertex_transform(deform_vert, tgt_mtx)

        deform_rast, _ = self.ctx.rasterize(
            deform_vpos_clip, mesh.t_pos_idx, (height, width)
        )
        deform_mask = deform_rast[..., 3:] > 0
        deform_mask_aa = self.ctx.antialias(
            deform_mask.float(), deform_rast, deform_vpos_clip, mesh.t_pos_idx
        )

        tgt_w2c = kwargs["tgt_w2c"]
        deform_v_incam = self.ctx.vertex_transform(deform_vert, tgt_w2c)
        deform_vz = -deform_v_incam[0, :, 2:3]
        # pdb.set_trace()
        deform_depth, _ = self.ctx.interpolate_one(
            deform_vz, deform_rast, mesh.t_pos_idx
        )
        deform_depth_aa = self.ctx.antialias(
            deform_depth, deform_rast, deform_vpos_clip, mesh.t_pos_idx
        )
        ...
        # pdb.set_trace()
        deform_normal, _ = self.ctx.interpolate_one(
            deform_v_nrm, deform_rast, mesh.t_pos_idx
        )
        ...
        out.update(
            {
                # "opacity": mask_aa,
                "deform_opacity": deform_mask_aa,
                "mesh": mesh,
                "deform_depth": deform_depth_aa,
                "deform_normal": gb_normal_aa,
            }
        )
        ...
        deform_pos, _ = self.ctx.interpolate_one(
            mesh.v_pos, deform_rast, mesh.t_pos_idx
        )
        ...
        # )
        # gb_rgb_fg, _ = self.ctx.interpolate_one(
        #     deform_field.contiguous(), rast, mesh.t_pos_idx
        # )
        ...
        out.update(
            {
                ...
                "deform_albedo": deform_albedo_fg,
            }
        )
