...
class CustomMesh(BaseExplicitGeometry):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.shape_init.startswith("mesh:"):
            ...
            attr_init = torch.zeros((v_pos.shape[0],12)).to(self.device)
            attr_init[:,0]=1.0
            attr_init[:,4]=1.0
            attr_init[:,8]=1.0
            self.vertex_attribute = torch.nn.parameter.Parameter(attr_init,requires_grad=True)
