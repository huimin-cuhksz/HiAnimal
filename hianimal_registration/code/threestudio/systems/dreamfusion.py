...
class DreamFusion(BaseLift3DSystem):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # [(name, param.requires_grad) for name, param in self.geometry.named_parameters()]

        # import pdb; pdb.set_trace()

        # img = out["comp_rgb"].detach().cpu().numpy()
        # # save img
        # img_pil = Image.fromarray((img[0] * 255).astype(np.uint8))

        # print(f"saving it{self.true_global_step}.png")
        # img_pil.save(f"it{self.true_global_step}.png")
